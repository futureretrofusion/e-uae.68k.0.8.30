...
static void dump_resetvec(const char *tag)
{
    uae_u32 sp0  = get_long(0x000000);
    uae_u32 pc0  = get_long(0x000004);
    uae_u32 spf8 = get_long(0xF80000);
    uae_u32 pcf8 = get_long(0xF80004);

    write_log("RESETVEC[%s] 0:SP=%08x PC=%08x  F8:SP=%08x PC=%08x\n",
              tag, sp0, pc0, spf8, pcf8);

    for (int i = 0; i < 16; i += 4)
        write_log("DUMP[0x%06x]=%08x\n", i, get_long(i));
    for (int i = 0; i < 16; i += 4)
        write_log("DUMP[0x%06x]=%08x\n", 0xF80000 + i, get_long(0xF80000 + i));
}
