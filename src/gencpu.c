 ...
 * Copyright Future Rettro Fusion 2025
 */
...
{
    /* keep a stable PC-offset snapshot for exception3 reporting (matches genamode2) */
    int m68k_pc_offset_last = m68k_pc_offset;
    ...
    switch (mode) {
     ...
     /* Effective-address destinations: ensure proper exception-3 on odd writes,
        mirroring the load-side check in genamode2 (size != byte). */
     case Aind:
     case Aipi:
     case Apdi:
     case Ad16:
     ...
     case PC16:
     case PC8r:
        if ((using_prefetch || using_ce) && using_exception_3 && size != sz_byte) {
            printf ("\tif (UAE_UNLIKELY(%sa & 1)) {\n", to);
            printf ("\t\texception3 (opcode, m68k_getpc(regs) + %d, %sa);\n",
                    m68k_pc_offset_last, to);
            printf ("\t\tgoto %s;\n", endlabelstr);
            printf ("\t}\n");
            need_endlabel = 1;
            start_brace ();
