...
void save_options (FILE *f, const struct uae_prefs *p, int type)
{
	...
	cfgfile_write (f, "gfx_hfact=%d\n", p->gfx_hfact); /* upscale factor width */
	cfgfile_write (f, "gfx_linefact=%d\n", p->gfx_linefact);/* upscale factor height */
...
static int cfgfile_parse_host (struct uae_prefs *p, char *option, char *value)
{
	...
	*tmpp = '\0';
	...
	|| cfgfile_intval (option, value, "gfx_hfact", &p->gfx_hfact, 1)
	|| cfgfile_intval (option, value, "gfx_linefact", &p->gfx_linefact, 1))
	return 1;
...
int parse_cmdline_option (struct uae_prefs *p, char c, char *arg)
{
	...
	/* New meanings:
	 *       H=1 : window (no requester)
	 *       H=2 : always show screenmode requester
	 *       H=3 : try config/env screenmode (amiga_uaesm / UAESM), if invalid -> requester
	 */
	if (p->amiga_screen_type < 1 || p->amiga_screen_type > 3) {
		write_log("Bad screen-type selected. Use H=1 (window), H=2 (requester), H=3 (uaesm->requester). Defaulting to window.\n");
		p->amiga_screen_type = 1;
	}
	#endif
	break;
