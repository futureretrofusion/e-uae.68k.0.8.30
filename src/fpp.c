...
static const double UAE_LN2     = 0.693147180559945309417232121458176568;   /* ln(2) */
static const double UAE_INV_LN2 = 1.442695040888963407359924681001892137;   /* 1/ln(2) */
static const double UAE_LN10    = 2.302585092994045901093613792909309267;   /* ln(10) */
static const double UAE_LOG10E  = 0.434294481903251827651128918916605082;   /* log10(e) */
STATIC_INLINE fptype to_pack(uae_u32 w1, uae_u32 w2, uae_u32 w3);
...
STATIC_INLINE tointtype toint (fpp_rounding_mode mode, fptype src)
{
    ...
    switch (mode) {
        case FPP_ROUND_TO_NEAREST: return (tointtype)lrint(src);  /* ties-to-even, matches FPU */
        case FPP_ROUND_TO_ZERO:    return (tointtype)trunc(src);
        case FPP_ROUND_DOWN:       return (tointtype)floor(src);
        case FPP_ROUND_UP:         return (tointtype)ceil(src);
    }
#else
    switch (mode) {
...
# else
            return (tointtype)(src + (src >= 0.0 ? 0.5 : -0.5)); /* simple, old fallback */
# endif
        case FPP_ROUND_TO_ZERO:    return (tointtype)src;
        case FPP_ROUND_DOWN:       return (tointtype)floor(src);
        case FPP_ROUND_UP:         return (tointtype)ceil(src);
    }
#endif
...
/* Packed-decimal -> double (fast path, no sscanf).
   Layout:
     w1: [sign bit 31][exp sign bit 30][exp3:4][exp2:4][exp1:4][d0:4]
     w2: d1 d2 d3 d4 d5 d6 d7 d8  (nibbles, msb first)
     w3: d9 d10 d11 d12 d13 d14 d15 d16 (nibbles)
   Value = (+/-)(d0.d1...d16) * 10^E , where E = exp3*100 + exp2*10 + exp1, with sign.
*/
STATIC_INLINE fptype to_pack(uae_u32 w1, uae_u32 w2, uae_u32 w3)
{
    const int sign    = (w1 >> 31) & 1;
    const int expsign = (w1 >> 30) & 1;
    const int exp3    = (int)((w1 >> 24) & 0xF);
    const int exp2    = (int)((w1 >> 20) & 0xF);
    const int exp1    = (int)((w1 >> 16) & 0xF);

    int E = exp3 * 100 + exp2 * 10 + exp1;
    if (expsign) E = -E;

    /* Accumulate 17-digit integer M as double, then scale by 1e-16 */
    double Macc = (double)(w1 & 0xF);  /* d0 */

    uae_u32 words[2] = { w2, w3 };
    for (int wi = 0; wi < 2; ++wi) {
        uae_u32 W = words[wi];
        for (int sh = 28; sh >= 0; sh -= 4) {
            int d = (int)((W >> sh) & 0xF);
            if (d > 9) d = 0;          /* clamp non-BCD nibbles just in case */
            Macc = Macc * 10.0 + (double)d;
        }
    }

    const double mant  = Macc * 1.0e-16;
#if defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L
    const double scale = pow(10.0, (double)E);
#else
    const double UAE_LN10 = 2.302585092994045901093613792909309267;
    const double scale = exp((double)E * UAE_LN10);
#endif
    const double v = mant * scale;
    return sign ? (fptype)(-v) : (fptype)v;
}
...
STATIC_INLINE int get_fp_value (uae_u32 opcode, uae_u16 extra, fptype *src)
{
	...
	if (UAE_LIKELY((extra & 0x4000) == 0)) {  /* register source is common */
...
STATIC_INLINE int put_fp_value (struct regstruct *regs, fptype value, uae_u32 opcode, uae_u16 extra)
{
	...
	if (UAE_LIKELY((extra & 0x4000) == 0)) {  /* register dest is common */
...
void fpp_opp (uae_u32 opcode, struct regstruct *regs, uae_u16 extra)
{
	...
	if ((extra & 0xfc00) == 0x5c00) {
		...
		break;
	    case 0x0d:
		regs->fp[reg] = UAE_INV_LN2;
		break;
	    case 0x0e:
		regs->fp[reg] = UAE_LOG10E;
		break;
	    case 0x0f:
		regs->fp[reg] = 0.0;
		break;
	...
	case 0x45:
		...
		fround (reg);
	    break;
		/* FLOGNP1: log(1+x) → log1p(x) is faster & more accurate near 0 */
	case 0x06: { /* FLOGNP1 */
	#if defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L
		regs->fp[reg] = log1p(src);
	#else
		regs->fp[reg] = log(src + 1.0);
	#endif
	} break;
	/* FETOXM1: exp(x) - 1  → expm1(x) is faster & more accurate near 0 */
	case 0x08: { /* FETOXM1 */
	#if defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L
		regs->fp[reg] = expm1(src);
	#else
	...
	case 0x11: { /* FTWOTOX */
	#if defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L
		regs->fp[reg] = exp2(src);
	#else
		regs->fp[reg] = exp(src * 0x1.62e42fefa39efp-1); /* ln2 */
	#endif
	} break;
	/* FTENTOX: 10^x  → exp10(x) if available, else exp(x*ln10) with const */
	case 0x12: { /* FTENTOX */
	#if defined(__GLIBC__) && defined(__GLIBC_MINOR__) /* exp10 is a GNU libm ext */
		regs->fp[reg] = exp10(src);
	#else
		regs->fp[reg] = exp(src * 2.3025850929940459011); /* ln(10) */
	#endif
	} break;
	case 0x14: /* FLOGN */
	    regs->fp[reg] = log (src);
	...
	case 0x15: /* FLOG10 */
	    ...
	    break;
	/* FLOG2: log base 2  → log2(x) */
	case 0x16: { /* FLOG2 */
	#if defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L
		regs->fp[reg] = log2(src);
	#else
		regs->fp[reg] = log(src) * 1.4426950408889634074; /* 1/ln(2) */
	#endif
	} break;
	case 0x18: /* FABS */
	case 0x58:
	...
	case 0x25: /* FREM */
	    ...
	    break;
		/* FSCALE: dst *= 2^trunc(src)  — integer exponent, fastest */
	case 0x26: { /* FSCALE */
		/* trunc() obeys current rounding mode; if you want toward-zero exactly, use (toint(FPP_ROUND_TO_ZERO, src)) */
	#if defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L
		int e = (int)trunc(src);
	#else
		int e = (int)((src >= 0.0) ? floor(src) : ceil(src));
	#endif
		regs->fp[reg] = scalbn(regs->fp[reg], e);  /* x * 2^e */
		break;
	}
	...
	case 0x34: case 0x35: case 0x36: case 0x37: {
		/* Portable: compute both separately */
		double s = sin(src);
		double c = cos(src);
		regs->fp[extra & 7] = c;
		regs->fp[reg]       = s;
