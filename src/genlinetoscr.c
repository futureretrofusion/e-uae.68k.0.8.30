...
static void out_linetoscr_mode (DEPTH_T bpp, HMODE_T hmode, int aga, CMODE_T cmode)
{
    ...
    if (bpp == DEPTH_16BPP && hmode != HMODE_DOUBLE) {
        outln (                "int rem;");
        outln (                "if (((uintptr_t)&buf[dpix]) & 2) {"); /* fix: uintptr_t for alignment test */
        ...
        outln (                "    return spix;");
        outln (                "rem = (((uintptr_t)&buf[stoppos]) & 2);"); /* fix: uintptr_t */
        outln (                "if (rem)");
        outln (                "    stoppos--;");
    }
    ...
    if (hmode == HMODE_DOUBLE) {
        if (bpp == DEPTH_8BPP) {
            /* fix: duplicate 8-bit pixel safely; avoid misaligned 16-bit store if address is odd */
            outln (        "    if (((uintptr_t)&buf[dpix]) & 1) {");
            outln (        "        buf[dpix++] = (uae_u8)(out_val & 0xFF);");
            outln (        "        buf[dpix++] = (uae_u8)(out_val & 0xFF);");
            outln (        "    } else {");
            outln (        "        out_val &= 0xFF;");
            outln (        "        out_val |= (out_val << 8);");
            outln (        "        *((uae_u16 *)&buf[dpix]) = (uae_u16) out_val;");
            outln (        "        dpix += 2;");
            outln (        "    }");
        } else if (bpp == DEPTH_16BPP) {
            /* fix: pack duplicate 16-bit pixel into 32-bit word, with safe fallback if address is 2 mod 4 */
            outln (        "    if (((uintptr_t)&buf[dpix]) & 2) {");
            outln (        "        buf[dpix++] = (uae_u16)(out_val & 0xFFFF);");
            outln (        "        buf[dpix++] = (uae_u16)(out_val & 0xFFFF);");
            outln (        "    } else {");
            if (do_bigendian)
                outln (    "        out_val = (out_val << 16) | (out_val & 0xFFFF);");
            else
                outln (    "        out_val = (out_val & 0xFFFF) | (out_val << 16);");
...
int main (int argc, char *argv[])
{
   ...
   outln (" */");
   /* fix: generated file needs uintptr_t for alignment checks */
