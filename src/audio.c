 ...
 /* ------------------------------------------------------------------ */
 /* FRF: Master output volume (software scaler in the mixer)
  * Ensures hotkey volume changes affect the final output on all backends.
  * 256 = 100% (fixed-point, Q8.8)
  * ------------------------------------------------------------------ */
 static int frf_mastervol_mul = 256;

 STATIC_INLINE void frf_update_mastervol_mul (void)
 {
     int v = currprefs.sound_volume;
     if (v < 0) v = 0;
     if (v > 100) v = 100;
     frf_mastervol_mul = (v * 256 + 50) / 100;
 }

 STATIC_INLINE uae_u32 frf_apply_mastervol (uae_u32 v)
 {
     if (frf_mastervol_mul == 256)
         return v;
     {
         long long sv = (long long)(int)v;
         sv = (sv * (long long)frf_mastervol_mul) >> 8;
         return (uae_u32)(int)sv;
     }
 }
 ...
 #define DO_CHANNEL_1(v, c) do { (v) *= audio_channel[c].vol; (v) = frf_apply_mastervol (v); } while (0)
 ...
 #define DO_CHANNEL_1(v, c) do { (v) = audio_channel[c].voltbl[(v)]; (v) = frf_apply_mastervol (v); } while (0)
 ...
 void audio_volume (int volume)
 {
     ...
     /* Compatibility:
      * - caller passes +1 / -1: treat as step (hotkeys)
      * - otherwise: treat as absolute percent (0..100)
      */
     if (volume == 1)
         v += 5;
     else if (volume == -1)
