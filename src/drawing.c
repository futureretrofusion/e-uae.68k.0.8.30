  ...
  * Copyright 2026 Future Retro Fusion
  */
...
static const char *numbers = { /* ugly  0123456789CHD%FRA. */
"+++++++--++++-+++++++++++++++++-++++++++++++++++++++++++++++++++++++++++++++-++++++-++++----++---+++++++++++++++++++++++++++++++++++++++++++"
"+xxxxx+--+xx+-+xxxxx++xxxxx++x+-+x++xxxxx++xxxxx++xxxxx++xxxxx++xxxxx++xxxx+-+x++x+-+xxx++-+xx+-+x++xxxx++xxxx++++xx++--------++xxxx++xxxx++"
"+x+++x+--++x+-+++++x++++++x++x+++x++x++++++x++++++++++x++x+++x++x+++x++x++++-+x++x+-+x++x+--+x++x+++x+++++x++x+++x++x+--------++x+++++x+++x+"
"+x+-+x+---+x+-+xxxxx++xxxxx++xxxxx++xxxxx++xxxxx+--++x+-+xxxxx++xxxxx++x+----+xxxx+-+x++x+----+x+-++xxx+++xxxx+++xxxx+--------++xxx+++xxxxx+"
"+x+++x+---+x+-+x++++++++++x++++++x++++++x++x+++x+--+x+--+x+++x++++++x++x++++-+x++x+-+x++x+---+x+x+++x+++++x+++x++x++x+----xx--++x+++++x+++++"
"+xxxxx+---+x+-+xxxxx++xxxxx+----+x++xxxxx++xxxxx+--+x+--+xxxxx++xxxxx++xxxx+-+x++x+-+xxx+---+x++xx++x+++++x+++x++x++x+----xx--++xxxx++x+++++"
"+++++++---+++-++++++++++++++----+++++++++++++++++--+++--++++++++++++++++++++-++++++-++++----------++++++++++++++++++++++++++++++++++++++++++"
};
...
static void draw_status_line (int line)
{
    ...
    /* --- Left-of-status label: "FFF C 2025" --- */
if (y >= TD_PADY && y - TD_PADY < TD_NUM_HEIGHT) {
    int tx, ty = y - TD_PADY;

    /* FFF(3) + space + C(1) + space + 2025(4) = 10 glyph cells */
    const int CELLS = 10;
    const int LABEL_GAP_PIX = TD_NUM_WIDTH * 2;   /* spacing before first LED box */
    const int total_w = CELLS * TD_NUM_WIDTH;

    tx = x_start - total_w - LABEL_GAP_PIX;
    if (tx < 0) tx = 0;

  /* indices: 0..9 digits, 10='C', 11='H', 12='D', 13='%', 14='F' */
    write_tdnumber (tx, ty, 14); tx += TD_NUM_WIDTH;   /* F */
    write_tdnumber (tx, ty, 15); tx += TD_NUM_WIDTH;   /* R */
    write_tdnumber (tx, ty, 14); tx += TD_NUM_WIDTH;   /* F */

    tx += TD_NUM_WIDTH;                                 /* space */

    write_tdnumber (tx, ty, 2);  tx += TD_NUM_WIDTH;   /* 2 */
    write_tdnumber (tx, ty, 0);  tx += TD_NUM_WIDTH;   /* 0 */
    write_tdnumber (tx, ty, 2);  tx += TD_NUM_WIDTH;   /* 2 */
    ...
    for (led = 0; led < NUM_LEDS; led++) {
        int side, pos = 0, num1 = -1, num2 = -1, num3 = -1, num4 = -1;
        int x, off_rgb = 0x000, on_rgb = 0x000, c, on = 0, am = 2;

        /* Map LED index (0..6) to our compact row */
        switch (led) {

        case 0: { /* IDLE (pos 0) */
            int idle = (gui_data.idle + 5) / 10;
            pos = 0;
            on_rgb = 0x000; off_rgb = 0x000;
            num1 = idle / 100;
            num2 = (idle - num1 * 100) / 10;
            num3 = idle % 10;
            num4 = (num1 == 0) ? 13 : -1; /* '%' glyph if 2 digits; omit if 3 digits */
            am = 3;
        } break;
        ...
        case 2: { /* POWER (pos 2) */
            ...
            on_rgb = 0xc00; off_rgb = 0x300;
        } break;

        case 3: { /* HD (pos 3) */
            pos = 3;
            on = gui_data.hd;
            on_rgb = (on == 2) ? 0xc00 : 0x00c;  /* write=red, read=blue */
            off_rgb = 0x003;
            num1 = -1; num2 = 11; num3 = 12;     /* 'H' 'D' */
        } break;

        case 4: { /* DF0 (pos 4) */
            int track = gui_data.drive_track[0];
            pos = 4;
            on_rgb = 0x0c0; off_rgb = 0x030;
            if (!gui_data.drive_disabled[0]) {
                num1 = -1;
                num2 = track / 10;
                num3 = track % 10;
                on = gui_data.drive_motor[0];
                if (gui_data.drive_writing[0])
                    on_rgb = 0xc00;
            }
        ...
        case 5: { /* DF1 (pos 5) */
            int track = gui_data.drive_track[1];
            pos = 5;
            on_rgb = 0x0c0; off_rgb = 0x030;
            if (!gui_data.drive_disabled[1]) {
                num1 = -1;
                num2 = track / 10;
                num3 = track % 10;
                on = gui_data.drive_motor[1];
                if (gui_data.drive_writing[1])
                    on_rgb = 0xc00;
            }
        } break;

        case 6: { /* FRAMESKIP (pos 6) */
            /* Two digits (00..99), flash bg RED on +, GREEN on - */
            int fs = gui_data.frameskip;
            if (fs < 0)  fs = 0;
            if (fs > 99) fs = 99;
            pos = 6;
            am  = 2;
            ...
            num2 = fs / 10;
            num3 = fs % 10;
            on_rgb  = (gui_data.frameskip_flash_polarity == 2) ? 0xc00
                    : (gui_data.frameskip_flash_polarity == 1) ? 0x0c0
                    : 0x000;
            off_rgb = 0x000;
            on = (gui_data.frameskip_flash_ticks > 0);
            /* decrement pulse once per status repaint (top scanline) */
            if (y == 0 && gui_data.frameskip_flash_ticks > 0)
                gui_data.frameskip_flash_ticks--;
        } break;

        } /* switch */

        /* Common LED box rendering */
