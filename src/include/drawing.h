 ...
 * (patched: global color reduction hook via CONVERT_RGB)
 *
...
/*
 * Global color-reduction hook for native (non-RTG) paths.
 * Define exactly one of:
 *   -DUAE_FORCE_TINYPALETTE : quantize to tiny palette (use AGA_TARGET_COLORS=32 or 16)
 *   -DUAE_FORCE_RGB444      : quantize to 12-bit RGB (4-4-4)
 * Otherwise, default to the original UAE AGA conversion (or 0 on OCS/ECS).
 */
#if defined(UAE_FORCE_TINYPALETTE)

/* Tiny palette quantizer (expects tiny_palette.h & AGA_TARGET_COLORS) */
#include "tiny_palette.h"
#ifdef CONVERT_RGB
#undef CONVERT_RGB
#endif
#define CONVERT_RGB(v24_) aga24_to_tiny_xcolor((v24_))

#elif defined(UAE_FORCE_RGB444)

/* 24-bit -> 12-bit (RGB444) mapped through xcolors[0..0xFFF] */
#ifdef CONVERT_RGB
#undef CONVERT_RGB
#endif
#define CONVERT_RGB(v24_) \
    ( xcolors[ \
        (((((v24_) >> 16) & 0xFF) >> 4) << 8) | \
        (((((v24_) >>  8) & 0xFF) >> 4) << 4) | \
        (((((v24_) >>  0) & 0xFF) >> 4) << 0) ] )

#else  /* no forced reduction: keep original behavior */
#ifdef AGA
...
#endif
#endif /* forced reduction switch */

/* Expand 12-bit ECS RGB (0xRGB, 4 bits each) to 24-bit 0xRRGGBB */
STATIC_INLINE uae_u32 ecs12_to_aga24 (int c)
{
    int r = (c >> 8) & 0x0F;
    int g = (c >> 4) & 0x0F;
    int b = (c >> 0) & 0x0F;
...
/* Chipset path color fetch */
STATIC_INLINE xcolnr getxcolor (int c)
{
#if defined(UAE_FORCE_TINYPALETTE)
    if (currprefs.chipset_mask & CSMASK_AGA)
        return aga24_to_tiny_xcolor((uae_u32)c);        /* AGA: c is 24-bit */
    else
        return aga24_to_tiny_xcolor(ecs12_to_aga24(c));  /* ECS: expand then quantize */

#elif defined(UAE_FORCE_RGB444)
    if (currprefs.chipset_mask & CSMASK_AGA) {
        uae_u32 v = (uae_u32)c;                          /* AGA: 24-bit */
        int r4 = ((v >> 16) & 0xFF) >> 4;
        int g4 = ((v >>  8) & 0xFF) >> 4;
        int b4 = ((v >>  0) & 0xFF) >> 4;
        return xcolors[(r4 << 8) | (g4 << 4) | (b4)];
    } else {
        /* ECS palette is already 12-bit */
        return xcolors[c & 0x0FFF];
    }
