...
#include <stdio.h> /* for printf used by fuzzy_memset helpers */
...
struct uae_prefs {
    ...
    /* AmigaOS: force a specific ScreenMode DisplayID + depth, like "0x50191101:15" */
    char amiga_uaesm[64];
    ...
    int gfx_xcenter;
    int gfx_ycenter;
    int gfx_hfact;      /* 1..4 : horizontal integer scale */
    int gfx_linefact;   /* 1..4 : vertical integer scale (replaces linedbl internally) */
    ...
    /* ===== Runtime color depth controls (added) =====
     * color_bits_per_channel: 1..8 (1=~8 colors total, 4=RGB444, 8=24-bit)
     * color_dither: 0/1 (renderer may consult this; default 1 = allow)
     */
    int color_bits_per_channel;
    int color_dither;
    ...
    int  amiga_use_grey;
    int  amiga_use_dither; /* AmigaOS driver-level dithering toggle */
#endif
