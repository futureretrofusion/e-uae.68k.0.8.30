...
static char savestate_base[MAX_DPATH];   /* full path without _slot/.uss */
...
{
#ifdef HAVE_UAE_STRLCPY
    uae_strlcpy(savestate_base, filename, sizeof(savestate_base));
#else
    strncpy(savestate_base, filename, sizeof(savestate_base) - 1);
    savestate_base[sizeof(savestate_base) - 1] = '\0';
#endif

    /* Strip trailing ".uss" (exact match, case-sensitive) */
    char *dot = strrchr(savestate_base, '.');
    if (dot && strcmp(dot, ".uss") == 0)
        *dot = '\0';

    /* Strip trailing "_<digits>" slot suffix if present */
    char *us = strrchr(savestate_base, '_');
    if (us) {
        const char *p = us + 1;
        int alldigits = (*p != '\0');
        while (*p) { if (*p < '0' || *p > '9') { alldigits = 0; break; } p++; }
        if (alldigits) *us = '\0';
    }

    /* Only update savestate_fname when no restore is already armed.
     * cfgfile.c sets savestate_fname + STATE_DORESTORE for statefile=;
     * overwriting it here would replace the intended file with "uae_quick.uss". */
    if (savestate_state != STATE_DORESTORE)
        snprintf(savestate_fname, sizeof(savestate_fname), "%s.uss", savestate_base);

    switch (mode) {
    case 1: savestate_docompress = 1; savestate_specialdump = 0; break; /* compressed */
    case 2: savestate_docompress = 0; savestate_specialdump = 0; break; /* uncompressed */
    case 3: savestate_docompress = 0; savestate_specialdump = 1; break; /* RAM dump */
    case 4: savestate_docompress = 0; savestate_specialdump = 2; break; /* audio dump */
    default:savestate_docompress = 0; savestate_specialdump = 0; break;
    }
}
...
{
    /* Rebuild target name from stable base each time */
    if (slot > 0)
        snprintf(savestate_fname, sizeof(savestate_fname), "%s_%d.uss", savestate_base, slot);
    else
        snprintf(savestate_fname, sizeof(savestate_fname), "%s.uss", savestate_base);

    if (save) {
        write_log("saving '%s'\n", savestate_fname);
        /* Do NOT force compression or call savestate_initsave() here */
