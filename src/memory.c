...
#define MAX_ILG 20
/* Use plain heap during RESTORE to avoid natmem/fixed mapping failures */
static void *restore_safe_alloc(size_t sz, const char *tag)
{
    void *p = malloc(sz);
    if (!p)
        write_log("RESTORE: heap alloc failed for %s (%lu bytes)\n", tag, (unsigned long)sz);
    return p;
}
...
static void allocate_memory (void)
{
    ...
    if (allocated_chipmem != currprefs.chipmem_size) {
        if (chipmemory) mapped_free(chipmemory);
        chipmemory = 0;

        allocated_chipmem = currprefs.chipmem_size;
        chipmem_mask = allocated_chipmem ? (allocated_chipmem - 1) : 0;

        if (allocated_chipmem) {
#ifdef SAVESTATE
            if (savestate_state == STATE_RESTORE)
                chipmemory = restore_safe_alloc(allocated_chipmem, "chip");
            else
#endif
                chipmemory = mapped_malloc(allocated_chipmem, "chip");

            if (!chipmemory) {
                write_log("Fatal error: out of memory for chipmem (%u bytes)\n", allocated_chipmem);
                allocated_chipmem = 0;
            }
        }
        clearexec();
    ...
    if (allocated_bogomem != currprefs.bogomem_size) {
        if (bogomemory) mapped_free(bogomemory);
        bogomemory = 0;

        allocated_bogomem = currprefs.bogomem_size;
        bogomem_mask = allocated_bogomem ? (allocated_bogomem - 1) : 0;

        if (allocated_bogomem) {
#ifdef SAVESTATE
            if (savestate_state == STATE_RESTORE)
                bogomemory = restore_safe_alloc(allocated_bogomem, "bogo");
            else
#endif
                bogomemory = mapped_malloc(allocated_bogomem, "bogo");

            if (!bogomemory) {
                write_log("Out of memory for bogomem (%u bytes)\n", allocated_bogomem);
                allocated_bogomem = 0;
            }
        }
        clearexec();
    ...
    if (allocated_a3000mem != currprefs.a3000mem_size) {
        if (a3000memory) mapped_free(a3000memory);
        a3000memory = 0;

        allocated_a3000mem = currprefs.a3000mem_size;
        a3000mem_mask = allocated_a3000mem ? (allocated_a3000mem - 1) : 0;

        if (allocated_a3000mem) {
#ifdef SAVESTATE
            if (savestate_state == STATE_RESTORE)
                a3000memory = restore_safe_alloc(allocated_a3000mem, "a3000");
            else
#endif
                a3000memory = mapped_malloc(allocated_a3000mem, "a3000");

            if (!a3000memory) {
                write_log("Out of memory for a3000mem (%u bytes)\n", allocated_a3000mem);
...
#ifdef SAVESTATE
    /* ===== SAFE RESTORE =====
       Only copy snapshot bytes if sizes match and file positions are valid.
       Otherwise, skip the copy (and memset the destination to 0 to avoid garbage). */
    if (savestate_state == STATE_RESTORE) {

        /* CHIP */
        if (chipmemory && chip_filepos) {
            if (allocated_chipmem == changed_prefs.chipmem_size) {
                write_log("RESTORE: chip OK (snap=%u cur=%u) @pos=%lu\n",
                          changed_prefs.chipmem_size, allocated_chipmem, (unsigned long)chip_filepos);
                restore_ram(chip_filepos, chipmemory);
            } else {
                write_log("RESTORE: chip SIZE MISMATCH (snap=%u cur=%u) — skipping copy, zeroing buffer\n",
                          changed_prefs.chipmem_size, allocated_chipmem);
                if (allocated_chipmem && chipmemory)
                    memset(chipmemory, 0, allocated_chipmem);
            }
        } else {
            write_log("RESTORE: chip skipped (ptr=%p pos=%lu)\n",
                      chipmemory, (unsigned long)chip_filepos);
        }

        /* BOGO (only if present now) */
        if (allocated_bogomem > 0) {
            if (bogomemory && bogo_filepos) {
                if (allocated_bogomem == changed_prefs.bogomem_size) {
                    write_log("RESTORE: bogo OK (snap=%u cur=%u) @pos=%lu\n",
                              changed_prefs.bogomem_size, allocated_bogomem, (unsigned long)bogo_filepos);
                    restore_ram(bogo_filepos, bogomemory);
                } else {
                    write_log("RESTORE: bogo SIZE MISMATCH (snap=%u cur=%u) — skipping copy, zeroing buffer\n",
                              changed_prefs.bogomem_size, allocated_bogomem);
                    memset(bogomemory, 0, allocated_bogomem);
                }
            } else {
                write_log("RESTORE: bogo skipped (ptr=%p pos=%lu)\n",
                          bogomemory, (unsigned long)bogo_filepos);
            }
        }
    }
#endif /* SAVESTATE */
