...
/* Fast, inline block conversion with 512 fast paths. */
static inline long adjust_blocks(long blocks, int fromsize, int tosize)
{
    if (__builtin_expect(tosize <= 0 || fromsize <= 0, 0))
        return -1;

    /* Common case: to 512 */
    if (__builtin_expect(tosize == 512, 1)) {
        if (fromsize == 512) return blocks;
        /* Power-of-two multiples get shift paths */
        if ((fromsize & (fromsize - 1)) == 0) {
            /* fromsize is a power of two */
            if (fromsize > 512)   return blocks * (fromsize >> 9);   /* 2^n / 512 */
            /* fromsize < 512 */
            int k = 9 - __builtin_ctz(fromsize);
            return (blocks + (blocks < 0 ? -1 : 1)) >> k;
        }
        /* Fallback */
        return (fromsize > 512)
            ? blocks * (fromsize / 512)
            : (blocks + (blocks < 0 ? -1 : 1)) / (512 / fromsize);
    }

    /* Generic fallback if caller ever uses non-512 tosize */
    if (fromsize == tosize) return blocks;
    if (fromsize >  tosize) return blocks * (fromsize / tosize);
...
/* Remove AllocVec/FreeVec; use stack InfoData. */
int get_fs_usage (const char *path, const char *disk, struct fs_usage *fsp)
{
    struct InfoData info; /* stack */
    int result = -1;
    ...
    if (Info(lock, &info)) {
        const int bsz = info.id_BytesPerBlock;
        const long total = info.id_NumBlocks;
        const long used  = info.id_NumBlocksUsed;
        fsp->fsu_blocks = adjust_blocks(total, bsz, 512);
        fsp->fsu_bfree  = fsp->fsu_bavail =
            adjust_blocks(total - used, bsz, 512);
